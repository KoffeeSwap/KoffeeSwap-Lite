...
import { TokenAmount, Token, ChainId, Percent, JSBI } from '@koffeeswap/sdk'
...
describe('utils', () => {
  describe('#getKccScanLink', () => {
    it('correct for tx', () => {
      expect(getKccScanLink(1, 'abc', 'transaction')).toEqual('https://etherscan.io/tx/abc')
    })
    it('correct for token', () => {
      expect(getKccScanLink(1, 'abc', 'token')).toEqual('https://etherscan.io/token/abc')
    })
    it('correct for address', () => {
      expect(getKccScanLink(1, 'abc', 'address')).toEqual('https://etherscan.io/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getKccScanLink(2, 'abc', 'address')).toEqual('https://etherscan.io/address/abc')
    })
    it('ropsten', () => {
      expect(getKccScanLink(3, 'abc', 'address')).toEqual('https://ropsten.etherscan.io/address/abc')
    })
    it('enum', () => {
      expect(getKccScanLink(ChainId.TESTNET, 'abc', 'address')).toEqual('https://rinkeby.etherscan.io/address/abc')
    })
  })
