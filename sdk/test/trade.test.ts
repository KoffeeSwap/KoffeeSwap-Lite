...
describe('Trade', () => {
  ...
  const pair_weth_0 = new Pair(
    new TokenAmount(WKCS[ChainId.MAINNET], JSBI.BigInt(1000)),
  ...
  it('can be constructed with ETHER as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, KCS),
      CurrencyAmount.ether(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(KCS)
  })
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(KCS)
      expect(result[0].route.path).toEqual([WKCS[ChainId.MAINNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(KCS)
      expect(result[1].route.path).toEqual([WKCS[ChainId.MAINNET], token0, token3])
    ...
    it('works for ETHER currency output', () => {
      ...
      expect(result[0].route.path).toEqual([token3, token0, WKCS[ChainId.MAINNET]])
      expect(result[0].outputAmount.currency).toEqual(KCS)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WKCS[ChainId.MAINNET]])
      expect(result[1].outputAmount.currency).toEqual(KCS)
    })
  })
