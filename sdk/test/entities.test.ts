...
import { ChainId, WKCS, TradeType, Rounding, Token, TokenAmount, Pair, Route, Trade } from '../src'
...
const WKCS = WKCS[ChainId.RINKEBY]
    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Pair', () => {
        pairs = [
          ...
          new Pair(
            new TokenAmount(tokens[2], decimalize(1, tokens[2].decimals)),
            new TokenAmount(WKCS, decimalize(1234, WKCS.decimals))
          )
        ]
      })
      ...
      it('Route', () => {
        ...
        expect(route.path).toEqual(tokens.concat([WKCS]))
        expect(route.input).toEqual(tokens[0])
        expect(route.output).toEqual(WKCS)
      })
      ...
      describe('Trade', () => {
          ...
          route = new Route(
            [
              new Pair(
                ...
                new TokenAmount(WKCS, decimalize(10, WKCS.decimals))
              )
            ],
            tokens[1]
          )
          const inputAmount = new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals))
          const expectedOutputAmount = new TokenAmount(WKCS, '1662497915624478906')
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          const outputAmount = new TokenAmount(WKCS, '1662497915624478906')
      ...
      it('TokenAmount', () => {
        const amount = new TokenAmount(WKCS, '1234567000000000000000')
